...
<?=CHtml::activeDropDownList($model, 'type', CHtml::listData(
        Tovartype::model()->findAll(),'id', 'type'),
        array(
            'empty' => '(Выберите...)',
            'class'=>'span3',
            'id'=>'dropd',
...
<br/>
<?=CHtml::activeLabel($model, 'caption');?>
<?=CHtml::activeTextField($model,'caption',array('style'=>'width: 570px'));?>
<br/>
<?=CHtml::activeLabel($model, 'description');?>
<?=CHtml::activeTextArea($model,'description',array('rows'=>20, 'style'=>'width: 570px'));?>
<br/>
<?=CHtml::submitButton('Сохранить', array('class'=>'btn btn-small'));?>
<?=CHtml::endForm();?>
...
Yii::app()->clientscript->registerScript('s',
        '
            $(\'#dropd\').change(function(){            
            $(\'#Tovartype_caption\').val(\'\');
            $(\'#Tovartype_description\').val(\'\');           
            $(\'form\').submit();
            });
        '
        );
