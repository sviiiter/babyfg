...
class RegistrationController extends Controller
{
	...
	public function actionRegistration() {
        //var_dump(isset($_POST['UserKids']));die();
    /*if ( isset($_POST['UserKids']) && ( $user_kids = array_diff ( $_POST['UserKids']['birth'], array( null))) ) {
      var_dump($_POST['UserKids']);die();
           ...
           var_dump($r);
            ...
            try {
              if ($model->save()) {
                ...
                if ( isset($_POST['UserKids']) && ( $user_kids = array_diff ( $_POST['UserKids']['birth'], array( null))) ) {                                    
                  $arPost = $_POST['UserKids'];
                  foreach ($user_kids as $field => $kid)  {
                    $kids = new UserKids;
                    $kids->user_id = $model->id;
                    $kids->birth = $kid;        
                    $kids->name = $arPost['name'][$field];
                    $kids->sex = $arPost['sex'][$field];
                    $kids->save();
                  }
                }                
                $transaction->commit();
